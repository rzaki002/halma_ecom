...
class OrderController extends Controller
{
    ...
    public function cobamidtrans(Request $request){
        ...
        $data_order_detail = $data_order->orderdetail; 
        $user = Customer::where('email',Auth::user()->email)->first();
        // $orderUser = Order::where('id_customer',$user->id)->get();
        $orderUser = Order::where('id',$order_id)->get();
        // dd($data_order_detail);
        
        $params = array(
            'transaction_details' => array(
                'order_id' => $order_id,
                'gross_amount' => $data_order_detail->qty * $data_order_detail->harga,
            )
        );
        \Midtrans\Config::$serverKey = 'SB-Mid-server-kcHyHpKTzKt_l9iOfyz-hpTK';
        // Set to Development/Sandbox Environment (default). Set to true for Production Environment (accept real transaction).
        \Midtrans\Config::$isProduction = false;
        // Set sanitization on (default)
        \Midtrans\Config::$isSanitized = true;
        ...
        $snapToken = \Midtrans\Snap::getSnapToken($params);
        return view('orders.midtrans',compact('snapToken','orderUser'));
    }
...
public function updatePembayaran(Request $request){
    ...
    if ($transaction_status=='settlement'){
        $orderan = Order::find($order_id)->update(['status_bayar'=> 'Lunas' ]);
        $pembayaran = Pembayaran::find($order_id)->update(['bukti_tf'=>$bukti]);

    }else{
        $orderan = Order::find($order_id)->update(['bukti_tf'=>$bukti,'status_bayar'=> 'Belum Lunas' ]);
        $pembayaran = Pembayaran::find($order_id)->update([
            ['bukti_tf'=>$bukti]
        ]);
    }
    ...
    {
        $order->delete();
        return redirect()->back()->with('success', 'Order Produk deleted susccesfully');
        // return redirect()->route('orders.index')
    ...
    public function ordercustomer(){
        ...
        return view('produks.ordercustomer', compact('orderUser'));
    }
}
